...
var atlasEntries = []*atlas.AtlasEntry{}

func init() {
	cborAtlas = atlas.MustBuild().WithMapMorphism(atlas.MapMorphism{atlas.KeySortMode_RFC7049})
}
...
func decodeBlock(block blocks.Block) (*Node, error) {
	var m interface{}
	cbor.UnmarshalAtlased(block.RawData(), m, cborAtlas)
...
func WrapObject(m interface{}, mhType uint64, mhLen int) (*Node, error) {
	...
	block, err := blocks.NewBlockWithCid(data, nil)
	if err != nil {
